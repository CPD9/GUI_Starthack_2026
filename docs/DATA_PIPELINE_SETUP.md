...
Then run the app in a second terminal (from repo root):
...
2. Start `npm run dev` from repo root.
...
```bash
# Next.js production gate (from repo root)
npm run build
...
You should **not** see files inside `data/raw/`.
