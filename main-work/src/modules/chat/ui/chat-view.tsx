...
export const ChatView = ({ userName }: Props) => {
  ...
  // Load a specific chat by ID (used when navigating from sidebar)
  ...
  const saveChat = async (newMessages: Message[]) => {
    ...
    const title = firstUserMessage?.content.slice(0, 50) + (firstUserMessage && firstUserMessage.content.length > 50 ? "..." : "") || "New Chat";
    ...
    if (!currentChatId) {
      ...
      // Add only the new messages to existing chat
  ...
  const handleSendMessage = async () => {
    ...
    // Simulate showing reasoning steps progressively
    ...
    // Auto-expand the reasoning for the latest message
  ...
  useEffect(() => {
    if (pendingVoiceMessage) {
      ...
      const sendVoiceMessage = async () => {
        ...
        setShowChat(true);
        setIsLoading(true);
        setPendingVoiceMessage(null); // Clear the pending message
  ...
  const transcribeAudio = useCallback(async (audioBlob: Blob) => {
    ...
    try {
      ...
      if (data.text) {
        // Set the transcribed text as pending for auto-send
  ...
  return (
    <div className="flex-1 flex min-h-0 bg-background">
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        {/* Collapsed Chat View */}
        {isChatCollapsed && showChat ? (
          <div className="flex-1 flex items-center justify-center p-4">
            <div className="text-center">
              <div className="bg-muted/50 rounded-full p-4 mb-4 inline-block">
                <MessageSquareIcon className="size-8 text-muted-foreground" />
              </div>
              <h3 className="font-medium text-foreground mb-2">Chat Collapsed</h3>
              <p className="text-sm text-muted-foreground mb-4">
                {messages.length} messages in this conversation
              </p>
              <Button onClick={() => setIsChatCollapsed(false)}>
          ...
          <div className="flex-1 flex flex-col items-center px-4 py-8 overflow-auto justify-start">
            {!showChat ? (
            ...
            <div className="w-full max-w-2xl mb-6">
              ...
              <div className="rounded-2xl shadow-lg border border-foreground bg-background p-2 flex items-end gap-2">
                <DropdownMenu>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <Button 
                    ...
                    className={`rounded-xl transition-colors ${isRecording ? 'bg-red-500/20 text-red-500 animate-pulse gap-2 px-3' : isTranscribing ? 'bg-blue-500/20 text-blue-500' : 'hover:bg-primary/10'}`}
                ...
                {SUGGESTION_BUBBLES.map((bubble) => {
                  ...
                  return (
                    <Button
                      ...
                      variant="outline"
                      asChild
                      className="rounded-full px-4 py-2 h-auto font-normal bg-background border-foreground text-foreground hover:bg-foreground hover:text-background transition-colors"
                    >
          ...
          <div className="w-full max-w-3xl flex-1 flex flex-col gap-4 h-full">
            {/* Messages Area */}
            <div 
              ref={messagesContainerRef}
              className="rounded-xl border border-border/50 p-4 flex-1 flex flex-col min-h-0 overflow-y-auto bg-background scroll-smooth"
              ...
              <div className="flex flex-col gap-4 mt-auto">
                ...
                {isLoading && (
                  <div className="flex gap-3">
                    <div className="shrink-0 size-8 rounded-full bg-background text-foreground border border-foreground flex items-center justify-center">
                      <BotIcon className="size-4" />
                    </div>
                    <div className="bg-background text-foreground border border-foreground rounded-2xl px-4 py-3">
                      <div className="flex items-center gap-2 mb-1">
